...
class PipelineDetect(Node):
    ...
    def image_callback(self, msg):
        try:
            ...
            # cv2.imshow("Received Msg", self.frame)
    ...
    def yolo_inference(self):
        ...
        # cv2.imshow("ShowFrame", show_frame)
        # cv2.waitKey(1)
        ...
        if self.final_mask is not None:
            ...
            cv2.circle(show_frame, upper_detect, radius=3, color=(255, 255, 255), thickness=-1)
            cv2.circle(show_frame, lower_detect, radius=3, color=(255, 255, 255), thickness=-1)
            cv2.line(show_frame, lower_detect, upper_detect, color=(255, 0, 0), thickness=2)
            ...
            cv2.circle(show_frame, upper_target, radius=4, color=(0, 0, 255), thickness=-1)
            cv2.circle(show_frame, lower_target, radius=4, color=(0, 0, 255), thickness=-1)

            cv2.line(show_frame, upper_detect, upper_target, color=(0, 0, 255), thickness=1)
            cv2.line(show_frame, lower_detect, lower_target, color=(0, 0, 255), thickness=1)
            ...
            cv2.putText(show_frame, str(upper_offset), upper_target, fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(255, 255, 255), thickness=1)
            cv2.putText(show_frame, str(lower_offset), lower_target, fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(255, 255, 255), thickness=1)
        ...
        cv2.putText(show_frame, fps_text, (5, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 1)

        # Show the processed frame with annotations
        cv2.imshow("Pipefollowing", show_frame)
        cv2.waitKey(1)
